...
public interface OtherRecordMapper {
    ...
    @Select("select * from earthquake.otherrecord where earthquake_id = (SELECT D_ID FROM earthquake.disasterinfo order by date desc limit 1) and date <=  (SELECT date FROM earthquake.disasterinfo order by date desc limit 1) + interval #{time} hour order by date desc;")
    @ResultMap(value = "otherRecordMap")
    List<OtherRecord> getRecentHourOtherRecord(@Param("time") int time);

    @Select("select * from earthquake.otherrecord where earthquake_id = (SELECT D_ID FROM earthquake.disasterinfo order by date desc limit 1) and date <=  (SELECT date FROM earthquake.disasterinfo order by date desc limit 1) + interval #{time} hour order by date desc limit #{pageNum}, #{limit};")
    @ResultMap(value = "otherRecordMap")
    List<OtherRecord> getRecentHourOtherRecordByPage(@Param("pageNum") int pageNum,@Param("limit")int limit,@Param("time") int time );
