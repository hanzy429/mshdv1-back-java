...
public interface WaterDisasterMapper {
    ...
    @Select("select * from earthquake.waterdisaster where earthquake_id = (SELECT D_ID FROM earthquake.disasterinfo order by date desc limit 1) and date <=  (SELECT date FROM earthquake.disasterinfo order by date desc limit 1) + interval #{time} hour order by date desc;")
    @ResultMap(value = "waterDisasterMap")
    List<WaterDisaster> getRecentHourWaterDisaster(@Param("time") int time);

    @Select("select * from earthquake.waterdisaster where earthquake_id = (SELECT D_ID FROM earthquake.disasterinfo order by date desc limit 1) and date <=  (SELECT date FROM earthquake.disasterinfo order by date desc limit 1) + interval #{time} hour order by date desc limit #{pageNum}, #{limit};")
    @ResultMap(value = "waterDisasterMap")
    List<WaterDisaster> getRecentHourWaterDisasterByPage(@Param("pageNum") int pageNum,@Param("limit")int limit,@Param("time") int time );
